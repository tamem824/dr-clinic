...
@section('content')
    <h3 class="mb-4">{{ __('endoscopy.edit') }}</h3>

    <div class="row">
        {{-- Main Form Column --}}
        <div class="col-md-8">
            <form action="{{ route('admin.endoscopies.update', $endoscopy->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="row mb-4">
                    <div class="col-md-4">
                        ...
                        <select id="patient_id" name="patient_id" class="form-control" required>
                            <option value="">{{ __('endoscopy.select_patient') }}</option>
                            @foreach($patients as $patient)
                                <option value="{{ $patient->id }}" {{ $patient->id == $endoscopy->patient_id ? 'selected' : '' }}>
                                    {{ $patient->fullname }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="date" id="performed_at" name="performed_at" class="form-control" required
                               value="{{ old('performed_at', $endoscopy->performed_at) }}">
                    </div>
                    <div class="col-md-4">
                        <label for="notes" class="text-primary">{{ __('endoscopy.notes') }}</label>
                        <textarea id="notes" name="notes" class="form-control" rows="3" placeholder="{{ __('endoscopy.notes_placeholder') }}">{{ old('notes', $endoscopy->notes) }}</textarea>
                    </div>
                </div>
                ...
                <div class="mb-4">
                    ...
                    @if($endoscopy->attachment)
                        <small class="text-muted">Current file: <a href="{{ asset('storage/' . $endoscopy->attachment) }}" target="_blank">View</a></small>
                    @endif
                </div>

                {{-- Hidden type input for form --}}
                <input type="hidden" name="type" id="endoscopyType" value="{{ old('type', $endoscopy->type) }}">

                {{-- Tabs for Upper and Lower Sections --}}
                <ul class="nav nav-tabs mb-3" id="sectionTabs" role="tablist">
                    @foreach($templates as $type => $sections)
                        @php
                            $tabClass = $type === 'upper' ? 'bg-primary text-white' : ($type === 'lower' ? 'bg-success text-white' : '');
                            $activeClass = ($endoscopy->type === $type) ? 'active' : '';
                        @endphp
                        <li class="nav-item" role="presentation">
                            <button class="nav-link {{ $activeClass }} {{ $tabClass }}" id="{{ $type }}-tab" data-bs-toggle="tab" data-bs-target="#tab-{{ $type }}" type="button" role="tab" aria-controls="tab-{{ $type }}" aria-selected="{{ $activeClass ? 'true' : 'false' }}">
                                {{ __('endoscopy.' . $type) }}
                            </button>
                        </li>
                    @endforeach
                ...
                <div class="tab-content">
                    @foreach($templates as $type => $sections)
                        <div class="tab-pane fade {{ ($endoscopy->type === $type) ? 'show active' : '' }}" id="tab-{{ $type }}" role="tabpanel" aria-labelledby="{{ $type }}-tab">
                            @foreach($sections as $section)
                                @php
                                    $data = $endoscopy->sections->where('template_id', $section->id)->first();
                                @endphp
                                <div class="card shadow-sm mb-3">
                                    <div class="card-body">
                                        ...
                                        <input type="hidden" name="sections[{{ $section->id }}][template_id]" value="{{ $section->id }}">
                                        ...
                                        <div class="row">
                                            <div class="col-md-4">
                                                ...
                                                <div class="input-group input-group-sm">
                                                    ...
                                                    <input type="text" name="sections[{{ $section->id }}][status]" class="form-control" placeholder="{{ __('endoscopy.status_placeholder') }}"
                                                           value="{{ old('sections.' . $section->id . '.status', $data->status ?? '') }}">
                                                </div>
                                            </div>
                                            <div class="col-md-8">
                                                <label class="text-secondary small">{{ __('endoscopy.description') }}</label>
                                                <div class="input-group input-group-sm">
                                                    <span class="input-group-text"><i class="bi bi-card-text"></i></span>
                                                    <textarea name="sections[{{ $section->id }}][description]" class="form-control" rows="1" placeholder="{{ __('endoscopy.description_placeholder') }}">{{ old('sections.' . $section->id . '.description', $data->description ?? '') }}</textarea>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                ...
                {{-- Buttons to save with proper type --}}
